...
from control_room_handover.adapters._review_payload import (
    result_to_review,
)
...
def _handover_service(container: Any) -> HandoverService:
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    brief = sample_cases.brief_with_pii()
    payload = repr(
        result_to_review(brief, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT).to_payload()
    )
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    brief = _handover_service(local).build_handover(
        sample_cases.HANDOVER_REQUEST, actor=sample_cases.ACTOR
    )
    assert brief.requires_human_review is True
    assert brief.citations, "an offline answer must still be cited"
    assert brief.scorecard.feeds, "the scorecard must carry a feed card"
    assert local.review_router.route(brief, maker=sample_cases.ACTOR)

    onprem = build_container(local_settings(profile="onprem"))
    with pytest.raises(NotImplementedError):
        _handover_service(onprem).build_handover(
            sample_cases.HANDOVER_REQUEST, actor=sample_cases.ACTOR
        )
