...
from control_room_handover.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "CONTROLROOM_AUDIT_ANCHOR"


def _handover_service(container: Container) -> HandoverService:
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    service = _handover_service(container)
    for shift in ("asia-day", "asia-night", "emea-day"):
        service.build_handover(
            sample_cases.HandoverRequest(shift_id=shift, as_of="2026-08-07"),
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    ports = build_container(local_settings())
    truncated = HandoverService(
        ops_feeds=ports.ops_feeds,
    ...
    with pytest.raises(AuditChainError):
        truncated.build_handover(sample_cases.HANDOVER_REQUEST, actor=sample_cases.ACTOR)
