...
def test_the_demo_data_is_obviously_fictional() -> None:
    pii_brief = demo._pii_brief()
    blob = " ".join(
        [demo.ACTOR, demo.TENANT, demo.PLANTED_NRIC]
        + [citation.snippet for citation in pii_brief.citations]
    )
    ...
    # The planted national id is a synthetic value that exists only to prove redaction happened.
    assert demo.PLANTED_NRIC in blob
...
def test_the_renderer_escapes_content_rather_than_interpolating_it() -> None:
    state = {
        ...
        "region": "r",
        "step_count": 1,
        "totals": {"briefs": 0, "routed": 0, "chain_ok": True},
        "steps": [
            {
                "key": "k",
                "label": "l",
