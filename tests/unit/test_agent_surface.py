...
from control_room_handover.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    build_agent_card,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "control-room-handover"
...
def test_the_agent_path_routes_a_handover_rather_than_only_flagging_it() -> None:
    result = build_shift_handover(
        sample_cases.HANDOVER_REQUEST.shift_id,
        sample_cases.HANDOVER_REQUEST.as_of,
        actor=sample_cases.ACTOR,
        tenant=sample_cases.TENANT,
        settings=local_settings(),
    )
    assert result["requires_human_review"] is True
    assert result["review_ref"], "the agent surface flagged a handover it never routed"
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    The masking is a recursive redaction of every string on the way out. A handover brief carries
    no per-case PII, so the guarantee is proved on the redaction pass directly: a planted
    identifier anywhere in the structure is masked before it can reach a model.
    """
    from control_room_handover.agent import tools

    nric = sample_cases.PLANTED_NRIC
    payload = tools._redacted({"note": f"break tied to NRIC {nric}", "nested": [nric]})
