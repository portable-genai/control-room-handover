...
from control_room_handover.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from control_room_handover.ports.identity import (
    VERIFIED,
...
_API_MODULE = "control_room_handover.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # managed IDENTITY binding is exercised without a live warehouse, model or TTS backend.
        *_rebind(
            "ops_feeds",
            "local.ops_feeds:LocalOpsFeedAdapter",
            "ops_feeds:OnPremOpsFeedAdapter",
        ),
        *_rebind(
            "generation",
            "local.generation:LocalDeterministicGenerationAdapter",
            "generation:OnPremGenerationAdapter",
        ),
        *_rebind("tts", "local.tts:LocalNoOpTtsAdapter", "tts:OnPremTtsAdapter"),
    ]
)
...
def _managed_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Any, audience: str | None) -> Any:
    ...
    supplies it. That expansion is three-state: ``None`` here means the variable is UNSET, which
    takes the written default (the empty string, i.e. no audience configured). It is NOT the same
    as setting the variable to empty, which the loader refuses outright.
    """
...
def test_a_malformed_assertion_from_a_lan_peer_answers_401_and_never_500(
    ...
    with TestClient(module.app, client=(LAN_PEER, 50000)) as client:
        resp = client.post(
            "/v1/handover",
            json={"shift_id": "asia-day", "as_of": "2026-08-07"},
...
def test_an_unconfigured_audience_answers_503_with_the_variable_named(
    ...
    The variable is UNSET, which is what "nobody configured an audience" is. Setting it to empty
    is a different state that the settings loader refuses before the app is built at all.
    """
