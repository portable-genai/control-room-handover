...
def test_an_unset_env_var_takes_the_files_written_default(
    ...
    """Three-state expansion, the open direction: nobody chose, so the written default stands."""
...
def test_an_env_var_set_to_empty_is_refused_rather_than_resolving_to_empty(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The middle state REFUSES; resolving it to the empty string is wrong.

    ``${VAR:-default}`` is ``setting_or_default(name, default)`` one layer down, so it must obey
    the same rule. Resolving an emptied variable to empty makes ``${VAR:-http://audit:8080}`` with
    ``VAR=""`` indistinguishable from ``${VAR:-}``: the operator who emptied it gets a value nobody
    reviewed, and for a base URL, an allowlist or a path the empty string is the permissive branch.
    The loader is the only layer that still knows a default was written, so it refuses here.
    """
    path = tmp_path / "settings.yaml"
    path.write_text("region: ${TEMPLATE_TEST_REGION:-fallback-region}\n", encoding="utf-8")
