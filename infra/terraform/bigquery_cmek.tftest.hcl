...
run "every_table_declares_the_datasets_key" {
  ...
  assert {
    condition     = length(google_bigquery_dataset.ops_worklist.default_encryption_configuration) == 1
    error_message = "The dataset must declare the CMEK default; without it the tables below have no key to agree with."
  }

  assert {
    condition     = length(google_bigquery_table.recon_breaks_export.encryption_configuration) == 1
    error_message = "google_bigquery_table.recon_breaks_export must declare the key the dataset stamps on it; an undeclared block reads as a removal at the next plan, which REPLACES the table and destroys every row it holds."
  }

  assert {
    condition     = length(google_bigquery_table.disputes_export.encryption_configuration) == 1
    error_message = "google_bigquery_table.disputes_export must declare the key the dataset stamps on it; an undeclared block reads as a removal at the next plan, which REPLACES the table and destroys every row it holds."
  }
