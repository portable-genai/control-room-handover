# Control Room Handover (F5)

Ops control-room SLA scorecard and shift handover over deterministic STP ops feeds.
...
control_room_handover handover asia-day 2026-08-07
```

## What it does

It reads the F1 (`recon-breaks-engine`) and F2 (`disputes-chargebacks-manager`) ops-worklist
export snapshots, computes a control-room scorecard with a pure deterministic engine (queue depth,
backlog aging, SLA breach counts and rate from the feeds' clock states, throughput, drain ratio,
capacity call-outs against a configured staffing baseline, and robust-z anomalies), and drafts a
shift-handover brief. The model narrates the scorecard and produces no number of its own: an
invented figure is discarded and a deterministic summary used instead. A handover always requires
the incoming shift lead's acknowledgement, so the brief is routed for sign-off (rule R8). Only the
two feeds that exist in this wave are bound; F3 and F4 join later as configuration, not code (see
`docs/ops-metrics-contract.md`).
...
One env var, `CONTROLROOM_PROFILE`, selects the adapter family:
...
`CONTROLROOM_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`CONTROLROOM_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
| `pii-kit` | the jurisdiction PII pattern pack the handover service redacts with |
| `speech-lexicon-kit` | the `TextToSpeechPort` types for the optional spoken handover brief |
...
**Name the profile.** `CONTROLROOM_PROFILE` has no default. Leaving it unset is
