# ARCHITECTURE: Control Room Handover (F5)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`CONTROLROOM_PROFILE`) swaps the
...
## Layout (`src/control_room_handover/`)
- `domain/` : pure stdlib, no cloud/framework imports. `kernel.py` (vertical-neutral types,
  `StrEnum` taxonomies from the commons), `models.py` (the control-room artifacts), `pii.py` (the
  jurisdiction pattern selection + order), `scorecard_engine.py` and `anomaly.py` (the
  deterministic engines), `acknowledgement.py` (the shift sign-off clock),
  `handover_service.py` (the orchestrator).
...
## Request pipeline (`HandoverService.build_handover`, then the caller)
