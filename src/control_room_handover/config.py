...
One env var (``CONTROLROOM_PROFILE``) selects the adapter family for every
...
from .ports.tts import TextToSpeechPort

_PROFILE_ENV = "CONTROLROOM_PROFILE"
_SETTINGS_ENV = "CONTROLROOM_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``CONTROLROOM_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CONTROLROOM_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "ops_feeds": {
        "local": f"{_PKG}.adapters.local.ops_feeds:LocalOpsFeedAdapter",
        "live": f"{_PKG}.adapters.local.ops_feeds:LocalOpsFeedAdapter",
        "gcp": f"{_PKG}.adapters.gcp.ops_feeds:CloudOpsFeedAdapter",
        "onprem": f"{_PKG}.adapters.onprem.ops_feeds:OnPremOpsFeedAdapter",
    },
    "generation": {
        "local": f"{_PKG}.adapters.local.generation:LocalDeterministicGenerationAdapter",
    ...
    "tts": {
        "local": f"{_PKG}.adapters.local.tts:LocalNoOpTtsAdapter",
        "live": f"{_PKG}.adapters.local.tts:LocalNoOpTtsAdapter",
        "gcp": f"{_PKG}.adapters.gcp.tts:CloudTtsAdapter",
        "onprem": f"{_PKG}.adapters.onprem.tts:OnPremTtsAdapter",
    },
}
...
def _expand(value: str) -> str:
    ...
    ``${VAR:-default}`` is ``setting_or_default(name, default)`` one layer down: a documented
    default that a variable may override. It obeys the same rule and delegates to the same
    helper, so the fleet carries one implementation of it rather than two. UNSET takes the
    written default; SET-AND-EMPTY names nothing and REFUSES with ``ConfiguredEmptyError``
    rather than resolving to the empty string, which for a base URL, an allowlist or a path is
    the permissive branch in several adapters. ``${VAR}`` with no ``:-`` carries the empty
    string as its written default, so unset yields empty and emptied still refuses.
    """
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CONTROLROOM_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "CONTROLROOM_REVIEW_ROUTING"
...
class Settings:
    ...
    project_id: str = ""
    #: Regional GCS bucket for optional generated voice briefs. Empty is an explicit
    #: managed-profile refusal when voice is requested; text handovers remain available.
    audio_bucket: str = ""
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            audio_bucket=str(data.get("audio_bucket") or ""),
...
class Container:
    ...
    @cached_property
    def ops_feeds(self) -> OpsFeedPort:
        adapter = self._bind("ops_feeds")
        assert isinstance(adapter, OpsFeedPort)
        return adapter
    ...
    @cached_property
    def tts(self) -> TextToSpeechPort:
        adapter = self._bind("tts")
