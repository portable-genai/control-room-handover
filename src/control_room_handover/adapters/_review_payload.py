...
def _kit_citations(result: HandoverBrief) -> tuple[KitCitation, ...]:
    seen: set[str] = set()
...
def result_to_review(result: HandoverBrief, *, maker: str, tenant: str = "") -> Review:
    """Build the review a producer submits to human-review-console when a result escalates."""
    return Review(
        action="control_room_handover:shift_handover",
        ...
        sod_group="control_room_handover-maker-checker",
        case_ref=result.subject,
        # Producer-owned, tenant-scoped key so a retried delivery is idempotent at the console.
        source_key=f"control-room-handover:{result.subject}:{result.severity.value}",
