"""FastAPI application for Control Room Handover (F5).
...
``CONTROLROOM_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CONTROLROOM_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real handover decision.
...
from .schemas import HandoverRequest as HandoverRequestModel
from .schemas import HandoverResponse, HealthResponse
...
_TOKEN_ENV = "CONTROLROOM_S2S_TOKEN"
_INSECURE_DEMO_ENV = "CONTROLROOM_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CONTROLROOM_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CONTROLROOM_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CONTROLROOM_S2S_AUDIENCE"
_API_HOST_ENV = "CONTROLROOM_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CONTROLROOM_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CONTROLROOM_PROFILE`` must never be allowed to
    ...
    ``CONTROLROOM_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CONTROLROOM_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "control-room-handover"
...
_APP_TITLE = "Control Room Handover"
...
#
# Note what is NOT in this expression: CONTROLROOM_S2S_TOKEN. A service
...
@app.post("/v1/handover", response_model=HandoverResponse, tags=["artifacts"])
def handover(
    request: HandoverRequestModel,
    principal: Annotated[Principal, Depends(get_principal)],
) -> HandoverResponse:
    """Build a shift-handover brief; the audit actor is the verified principal, not the body.
    ...
    # Rule R8: route for the incoming lead's sign-off in the same request that produced the brief.
    # The hand-off never fails an already-built, already-audited handover; the response says
    # what happened to it instead (the fleet's runtime-control contract).
    routing = RecordingReviewRouter(container.review_router)
    review_ref = routing.route(brief, maker=principal.actor, tenant=principal.tenant)
    return HandoverResponse.from_domain(
        brief, review_ref=review_ref, review_routing=routing.outcome.value
    )
