...
class AgentCard:
    ...
    provider: str = "control-room-handover"
    skills: tuple[AgentSkill, ...] = field(default_factory=tuple)


SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="build_shift_handover",
        name="Shift handover",
        description=(
            "Read the F1 / F2 ops-feed snapshots, compute the control-room scorecard "
            "deterministically (queue depth, SLA breaches, drain ratio, capacity call-outs, "
            "anomalies), narrate it, record an already-redacted audit event, and ROUTE the brief "
            "for the incoming shift lead's acknowledgement (rule R8). Every number is engine-"
            "computed, never a model's."
        ),
    ),
    AgentSkill(
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://control-room-handover.{settings.region}.internal.example/a2a"
