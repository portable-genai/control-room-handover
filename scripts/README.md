...
| `load_demo_book.py` | Loads the shipped fictional ops-worklist exports (`src/control_room_handover/data/demo_book/`) into the managed BigQuery dataset, so the deployment reads the same eight days the laptop reads from DuckDB rather than querying tables nothing created. It fills tables and never creates them, so the Terraform must be applied first, and it refuses to overwrite a dataset whose own `book_manifest` does not declare what it holds fictional. It loads into the schema each table already has rather than letting a truncating load autodetect one and replace it: an autodetected schema relaxes every mode to NULLABLE and reorders the columns, which makes the next `terraform plan` report the table as must-be-replaced and destroy the rows it holds. The rows are NOT rebased onto the load date: the window ends at the handover's own as-of, so a demo asks the deployment for the book's date and gets the same series. `--dry-run DIR` writes the NDJSON it would load and stops, needing no credentials and no `[gcp]` extra. |
...
| `rename_fork.py` | Not a demo script: the one-pass fork rebrand. Rewrites the package, the console-script name, the `CONTROLROOM` env prefix, the Terraform `name_prefix` stem and the git id in ONE simultaneous pass, so no rule can rewrite another's output. Previews by default and writes only with `--yes`. Stdlib-only and offline like everything else here. See `docs/ADOPTING.md` for the human decisions it deliberately leaves alone. |
