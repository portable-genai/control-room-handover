...
def _check_scorecard(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if int(facts.get("feeds", 0)) < 2:
        problems.append("fewer than the two wave feeds were scored")
    if int(facts.get("total_queue", 0)) <= 0:
        problems.append("the scorecard reported an empty backlog")
    if not facts.get("overall_severity"):
        problems.append("the scorecard produced no overall severity")
    return problems


def _check_handover(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if not facts.get("requires_human_review"):
        problems.append("a handover did not require sign-off")
    if not facts.get("review_ref"):
        problems.append("rule R8: a handover was flagged but not routed")
    if not facts.get("narration_grounded"):
        problems.append("the narration was not grounded in the engine figures")
    return problems
...
def _check_redaction(state: dict[str, Any]) -> list[str]:
    ...
    if facts.get("planted_identifier_leaked"):
        problems.append("the planted identifier reached the outbound review payload")
    if not facts.get("review_ref"):
        problems.append("rule R8: the brief with personal data was not routed")
    return problems
...
def _check_review_queue(state: dict[str, Any]) -> list[str]:
    ...
    if int(facts.get("pending", 0)) != int(state["totals"]["routed"]):
        problems.append("the queue does not hold every routed handover")
    return problems
...
def _check_audit(state: dict[str, Any]) -> list[str]:
    ...
    if int(facts.get("entries", 0)) < 3:
        problems.append("fewer audit records than handovers built")
    return problems
...
CHECKS: dict[str, Check] = {
    ...
    "scorecard": _check_scorecard,
    "handover": _check_handover,
