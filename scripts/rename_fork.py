...
Rewrites the python package name, the console-script name, the ``CONTROLROOM`` env-var prefix,
...
Markdown prose alone (pass ``--include-docs`` to sweep it too).
    ...
    # Apply it:
    python scripts/rename_fork.py --package acme_handover --cli acme-handover \
        --env-prefix ACME --resource acme-svc --yes
...
# --------------------------------------------------------------------------------------- #
_OLD_PACKAGE = "control_room_handover"
_OLD_CLI = "control_room_handover"
_OLD_ENV_STEM = "CONTROLROOM"
_OLD_RESOURCE = "f5-svc"
_OLD_DIST = "control-room-handover"
...
def main() -> int:
    ...
    parser.add_argument("--cli", required=True, help="new console-script name, e.g. acme-handover")
