...
Control Room Handover (F5): Ops control-room SLA scorecard and shift handover over deterministic STP ops feeds.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `control_room_handover`, environment prefix
`CONTROLROOM`, region `asia-southeast1`.
...
- `src/control_room_handover/domain/` is PURE stdlib. `kernel.py` holds the vertical-neutral
...
- `adapters/{local,gcp,onprem}/` are the three families. `local` is SDK-free and actually works;
  ...
  `local` laptop posture (`config.LAPTOP_PROFILES`), and narrates through the local open-weight
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `CONTROLROOM_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `CONTROLROOM_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `CONTROLROOM_PROFILE` (mentioning it in a refusal message is wanted, reading
