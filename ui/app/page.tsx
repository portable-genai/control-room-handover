...
const REVIEW_ROUTING_TEXT: Record<string, string> = {
  ...
  failed: "Could not reach the review console; this handover is not queued for review.",
  off: "Review routing is off in this deployment; this handover is not queued for review.",
};
...
export default function Home() {
  ...
  const [shift, setShift] = useState("asia-day");
  ...
  async function submit(event: React.FormEvent) {
    ...
    setFailed(false);
    try {
      const response = await fetch(API + "/v1/handover", {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Dev-Persona": persona },
        body: JSON.stringify({ shift_id: shift, as_of: asOf }),
      });
  ...
  return (
    <main>
      ...
      <p className="sub">
        {card?.description ??
          "Build a shift handover. The scorecard is deterministic and cited, and the brief is routed to the incoming shift lead for sign-off."}
      </p>
      ...
      <form onSubmit={submit}>
        ...
        <fieldset>
          <legend>The handover</legend>
          <label>
            Shift
            <input value={shift} onChange={(event) => setShift(event.target.value)} />
          </label>
          <label>
            As of (ISO date)
